        ...
        for v_idx, video in enumerate(dataset):
            ...
            for idx, (img, gt_bbox) in enumerate(video):
                ...
                if idx == frame_counter:
                    ...
                    clean_sample_init = image.astype(np.float64) - 128
                    ...
                    adversarial_sample = image.astype(np.float64) - 128
                ...
                if args.vis and idx > frame_counter:
                    cv2.polylines(img, [np.array(gt_bbox, np.int64).reshape((-1, 1, 2))],
                            True, (0, 255, 0), 3)
                    if cfg.MASK.MASK:
                        cv2.polylines(img, [np.array(pred_bbox, np.int64).reshape((-1, 1, 2))],
                                True, (0, 255, 255), 3)
                    else:
            ...
            # save results
            model_name = 'SiamRPN++(IoU_attack)'+args.snapshot.split('/')[-1].split('.')[0]
...
def orthogonal_perturbation(delta, prev_sample, target_sample):
    ...
    diff = (target_sample_temp - prev_sample_temp).astype(np.float64)
...
def forward_perturbation(epsilon, prev_sample, target_sample):
    perturb = (target_sample - prev_sample).astype(np.float64)
...
def get_diff(sample_1, sample_2):
    ...
    for i, channel in enumerate(sample_1):
        diff.append(np.linalg.norm((channel - sample_2[i]).astype(np.float64)))
